...
- [ ] [Move #192 code to Rumble Data Shard](https://app.asana.com/1/45238840754660/project/1210540875949204/task/1214430872139370) · High · WDK Indexer:TO DO · **AWAITING FRANCESCO — revert reason**
      > Asks to port [wdk-data-shard-wrk#192](https://github.com/tetherto/wdk-data-shard-wrk/pull/192) (wallet-anomaly-report CLI, Alex authored, merged 2026-04-20) to `rumble-data-shard-wrk` after Francesco reverted it in [#211](https://github.com/tetherto/wdk-data-shard-wrk/pull/211) on 2026-04-30 with no documented reason. Comment posted on the Asana ticket asking Francesco the revert reason, whether the migration has already run on Rumble (would make this obsolete), and the target branch. Repo-shape mismatch noted: rumble's `wallets.js` is 1666B vs 6693B in wdk, so the port likely also needs to add a scan method exposing `type` + `accountIndex`.
      local: `_tasks/04-may-26-WDK-1389-move-192-code-to-rumble-data-shard/`
- [ ] [Rumble - Security - Fix Tron Indexer High Vulnerabilities](https://app.asana.com/1/45238840754660/project/1210540875949204/task/1213478780310237) · High · WDK Indexer:TO DO · **VERIFIED FIXED ON main — awaiting close/repurpose decision**
      > Both referenced Dependabot alerts (#3 axios, #7 minimatch) are already `state: fixed` (2026-04-26). Verified on `wdk-indexer-wrk-tron@main` (commit `6e02432`): `npm audit --package-lock-only` → 0 critical/high/moderate, 10 lows all rooted at the still-open alert #1 (`elliptic <= 6.6.1`, no upstream patch). Comment posted on the ticket asking Francesco whether to close it or repurpose as parent for the broader "Rumble + dependents" sweep, and how to handle the elliptic low.
