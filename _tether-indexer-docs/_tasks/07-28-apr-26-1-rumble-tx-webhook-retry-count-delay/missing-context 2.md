...
- [x] ~~**API endpoint details:** `/wallet/v1/address-book` returning stale `tipping_enabled`~~ — **Resolved twice over:** (a) local code inspection (see `findings.md`) — zero matches across every `_INDEXER/` repo; on our side, `tipping_enabled` is derived on each call from the wallet row's `enabled` flag in the HyperDB shard via `rumble-app-node → rumble-ork-wrk → rumble-data-shard-wrk` (`getUserTipJar` / `getChannelTipJar`). (b) Alex confirmed verbally on 2026-04-28 — endpoint is Rumble's. **Source:** Patricio Vicens, 2026-02-25T19:41.
...
- [ ] **BE team handoff trail (added 2026-04-28):** Patricio's 2026-04-08 comment said "Reassigning to BE team", but no BE-side hypothesis or progress note exists in the ticket since. **Need from Alex:** any internal Slack thread / DM / standup note about ownership of this bug after FE handed it off — especially given the endpoint is Rumble's, did anyone open a Rumble-side ticket? **Source:** Patricio Vicens, 2026-04-08T17:17.
